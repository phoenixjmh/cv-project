...
const Experience = () => {
  ...
  const handleChange = (e, idx) => {
    if (e.target.id === "company-name") setName(e.target.value);
    if (e.target.id === "position") setPosition(e.target.value);
    if (e.target.id === "date-from") setFrom(e.target.value);
    if (e.target.id === "date-to") setTo(e.target.value);
    if (e.target.id === "job-description") setDescription(e.target.value);
  };
  ...
  return (
    <div className="expForm form-section">
      ...
      {modules.length > 0
        ...
        : null}
      {expModules > 0 ? displayForm(expModules) : null}
      <>
        {!isAdding ? (
          <button
            id="add-exp"
